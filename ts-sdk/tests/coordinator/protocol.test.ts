...
import { parseArgs } from "../../src/coordinator/runtime.js";
...
describe("runtime arg parser", () => {
    ...
    it("throws when --comm is missing", () => {
        expect(() =>
            parseArgs(["node", "bundle.mjs", "--logs=127.0.0.1:5002"]),
        ).toThrow(/Missing --comm/);
    });

    it("throws when --logs is missing", () => {
        expect(() =>
            parseArgs(["node", "bundle.mjs", "--comm=127.0.0.1:5001"]),
        ).toThrow(/Missing --logs/);
    });
});
