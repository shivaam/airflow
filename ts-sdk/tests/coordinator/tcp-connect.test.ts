...
describe("splitHostPort", () => {
    ...
    it("throws with the exact host:port message when there is no colon", () => {
        expect(() => splitHostPort("localhost")).toThrow(
            "Address must be host:port, got localhost",
        );
    });
...
describe("connectTcp", () => {
    it("rejects (does not throw synchronously) on a malformed address", async () => {
        await expect(connectTcp("nocolon")).rejects.toThrow(
            "Address must be host:port, got nocolon",
        );
    });
});
